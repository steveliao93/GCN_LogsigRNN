...
class Feeder(Dataset):
    def __init__(self, data_path, label_path, length_path, drop_rate,
    ...
    def load_data(self):
        ...
        for i in range(len(self.label)):
            ...
            tmp_deleted = np.delete(self.data[i], drop_index, 1)[:,:int(self.length[i].item()*(1-self.drop_rate))]
            try:
                ...
                continue
            # self.data[i] = np.concatenate((tmp_deleted, tmp_deleted[:,:len(drop_index)],tmp_deleted[:,:60]), axis=1)
