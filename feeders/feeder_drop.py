...
class Feeder(Dataset):
    def __init__(self, data_path, label_path,
                 ...
                 window_size=-1, normalization=False, debug=False, use_mmap=False, drop_rate=0.4):
        """
        :param data_path:
        :param label_path:
    ...
    def load_data(self):
        ...
        for i in range(len(self.label)):
            drop_index = np.random.choice(range(1,self.data.shape[2]), size=int(
                self.drop_rate * self.data.shape[2]), replace=False)
